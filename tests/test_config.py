...
def test_unknown_top_level_key_is_rejected():
    bad = {**VALID_MINIMAL, "debug_mode": True}
    with pytest.raises(ValidationError, match="debug_mode"):
        PipelineConfig(**bad)
...
def test_unknown_detector_type_is_rejected():
    bad = {
        **VALID_MINIMAL,
        "field_detector": {**VALID_MINIMAL["field_detector"], "type": "yolo_v99"},
    }
...
def test_reporter_requires_url_scheme():
